...
class App extends React.Component {
  ...
  async componentDidMount() {
    ...
    const { username, region, credential } = await this.getAccountSetting();
  ...
  async componentDidUpdate() {
    ...
    const { region, credential } = await this.getAccountSetting();
    ...
    let configFileId;
    try {
      configFileId = await this.uploadFile(
        JSON.stringify(this.composeConfig(region)),
        'sauce-runner.json',
        credential,
        region,
      );
  ...
  async getTunnels(credential, username, region) {
    ...
    Object.values(body).forEach((tunnels) => {
      tunnels.forEach((item) => tunnelIdentifiers.push(item.tunnel_identifier));
    });
  ...
  async startJob(credential, storage, runnerVersion, region) {
    ...
    try {
      resp = await fetch(
        `https://ondemand.${region}.saucelabs.com/wd/hub/session`,
        {
          method: 'POST',
          headers: {
            Accept: '*/*',
